...
// Interface dữ liệu API (Giống hệt Crypto)
...
// Props cho GoldDetail (Không cần assetImage nữa)
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="w-full max-w-5xl bg-foreground rounded-3xl shadow-2xl relative max-h-[85vh] flex flex-col">
        ...
        {fundLoading ? (
          ...
          <>
            {/* Header - HARDCODE ICON VÀNG TẠI ĐÂY */}
            <div className="p-8 pb-4 border-b border-text/10">
              <h2 className="text-2xl font-bold text-text mb-2">
                Chi tiết tài sản
              </h2>
              <div className="text-text/60 flex items-center gap-3">
                {/* Icon Coins cố định */}
                ...
                <span className="text-sm bg-text/10 px-2 py-0.5 rounded font-mono">
                ...
                <div className="bg-background rounded-xl p-4 border border-text/10">
                  ...
                  <p
                    className={`text-lg font-bold ${getChangeColor(
                      assetDetail.totalProfitAndLoss
                    )}`}
                  >
                    {assetDetail.totalProfitAndLoss >= 0 ? "+" : ""}
                    {formatPrice(assetDetail.totalProfitAndLoss)}
                  </p>
                </div>
                <div className="bg-background rounded-xl p-4 border border-text/10">
            ...
            <div className="flex-1 overflow-y-auto nice-scroll px-8 pb-8">
              ...
              {assetDetail &&
              assetDetail.listInvestmentDetailResponses.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-text/5">
                      {assetDetail.listInvestmentDetailResponses.map(
                        (transaction) => (
                          <tr
                          ...
                          >
                            ...
                            <td
                              className={`px-4 py-3 text-right text-sm font-semibold whitespace-nowrap ${getChangeColor(
                                transaction.currentProfit
                              )}`}
                            >
                              {transaction.currentProfit >= 0 ? "+" : ""}
                              {formatPrice(transaction.currentProfit)}
                            </td>

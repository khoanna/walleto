...
export default function TwoLineChart({ transactions }: TwoLineChartProps) {
  ...
  return (
    <div className="rounded-2xl p-3 sm:p-4 bg-background shadow-xl h-full flex flex-col">
      ...
      {/* 3. Cập nhật layout footer để chứa nút chi tiết */}
      <div className="mt-2 pt-2 border-t border-white/5 flex items-center justify-between">
        {/* Phần Tabs nằm bên trái */}
        ...
        {/* Nút Chi tiết nằm bên phải */}
        <button
          onClick={() => router.push("/dashboard/compare")}
          className="flex items-center gap-1 text-xs font-medium text-slate-400 hover:text-white transition-colors pl-2"
        >
          Chi tiết
          <ArrowBigRight size={18} className="fill-current" />
        </button>
      </div>
    </div>
  );
